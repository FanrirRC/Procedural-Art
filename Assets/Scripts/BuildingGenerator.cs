...
public class BuildingGenerator : MonoBehaviour
{
    ...
    public void BuildingSpawner(GameObject node1, GameObject node2, float roadThickness)
    {
        ...
        while (currentSpawnPos < Vector3.Distance(node1.transform.position, node2.transform.position))
        {
            ...
            GameObject basePrefab = buildGen[tempDecide].buildingBase[0];
            BuildingSegment segment = basePrefab.GetComponent<BuildingSegment>();

            if (segment == null)
            {
                Debug.LogError("Missing BuildingSegment script on: " + basePrefab.name);
                return;
            }

            float width = segment.segmentWidth;
    ...
    public IEnumerator LoadSegments(GameObject buildingParent)
    {
        for (int i = 0; i < Segments.Count; i++)
        {
            ...
            if (!spawnInstantHouses)
            {
                ...
                {
                    Collider[] hitColliders = Physics.OverlapBox(SpawnedSegment.transform.position, SpawnedSegment.GetComponentInChildren<Renderer>().bounds.size*1.1f / 4, Quaternion.identity, m_LayerMask);
                    if (hitColliders.Count() > 0)
                    {
                        DestroyImmediate(SpawnedSegment);
                        i += Segments[i].buildHeight;
